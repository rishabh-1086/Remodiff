...
class ScoreNetReMoDiff(nn.Module):
    """A time-dependent score-based model built upon U-Net architecture."""
    ...
    def __init__(self, marginal_prob_std, channels=[32, 64, 128, 256], embed_dim=256):
        ...
        # Encoding layers where the temporal resolution decreases
        self.conv1 = nn.Conv1d(32, channels[0], 3, padding=1, bias=False)
        self.attention_1 = TransformerEncoder(embed_dim=channels[0],
                                              num_heads=8,
                                              layers=2,
                                              attn_dropout=0.0,
        ...
        # Decoder
        self.tconv3 = nn.ConvTranspose1d(channels[2] + channels[2], channels[1], kernel_size=3, stride=2,
                                         padding=1, output_padding=1, bias=False)
        self.tconv3_cond = nn.ConvTranspose1d(channels[2], channels[1], kernel_size=3, stride=2,
                                              padding=1, output_padding=1, bias=False)
        self.attention_t3 = TransformerEncoder(embed_dim=channels[1],
                                              num_heads=8,
                                              layers=2,
    ...
    def forward(self, x, t, condition=None):
        ...
        # Decoding path
        h = self.tconv3(torch.cat([h3, h3], dim=1))
        if condition is not None:
            condition = self.tconv3_cond(condition)
            h_with_cond = self.attention_t3(h.permute(2, 0, 1), condition.permute(2, 0, 1), condition.permute(2, 0, 1))
            h += h_with_cond.permute(1, 2, 0)
...
def Euler_Maruyama_sampler(score_model,
    ...
    init_x = torch.randn(batch_size, 32, 3, device=device) \
             * marginal_prob_std(t)[:, None, None]  # Changed 48 to 3 by Rishabh
    ...
    with torch.no_grad():
        for time_step in time_steps:
            ...
            if condition is not None:
                perturbed_condition = condition + torch.randn(batch_size, 32, 3, device=device) * marginal_prob_std(batch_time_step)[:, None, None]  # Changed 48 to 3 by Rishabh
                #perturbed_condition = condition + torch.randn(batch_size, 32, 48, device=device) * marginal_prob_std(
                #    batch_time_step)[:, None, None]  # Changed 48 to 3 by Rishabh
